...
export default function Registro() {
    ...
    useEffect(() => {
        const fetchRoles = async () => {
            try {
                const response = await fetch('http://tu-backend-fastapi.com/roles');
                if (!response.ok) {
                ...
                setErrors(prev => ({ ...prev, submit: 'Error al cargar los roles' }));
            } finally {
    ...
    const validateForm = (): boolean => {
        ...
        if (!formData.id_sucursal.trim()) newErrors.id_sucursal = 'ID Sucursal requerido';
            ...
            <div className="min-h-screen flex flex-col">
                <div className="container max-w-md mx-auto flex-1 flex flex-col items-center justify-center px-2">
                    <div className="bg-white px-8 py-6 rounded shadow-md w-full">
                        <h1 className="mb-6 text-2xl font-bold text-center text-gray-600">Registro de Empleado</h1>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-gray-700 mb-1">ID Empleado*</label>
                                    <input
                                        name="id_empleado"
                                        value={formData.id_empleado}
                                        onChange={handleChange}
                                        className={`w-full p-3 border rounded text-gray-600 ${errors.id_empleado ? 'border-red-500' : 'border-black'}`}
                                ...
                                <div>
                                    <label className="block text-gray-700 mb-1">ID Sucursal*</label>
                                    <input
                                        name="id_sucursal"
                                        value={formData.id_sucursal}
                                        onChange={handleChange}
                                        className={`w-full p-3 border rounded text-gray-600 ${errors.id_sucursal ? 'border-red-500' : 'border-black'}`}
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={handleChange}
                                    className={`w-full p-3 border rounded text-gray-600 ${errors.id_rol ? 'border-red-500' : 'border-black'}`}
                                ...
                                {isLoadingRoles && <p className="text-sm text-gray-500 mt-1">Cargando roles...</p>}
                            </div>
                            ...
                            <button
                                ...
                                className={`w-full py-3 px-4 mt-6 rounded text-white font-medium ${isSubmitting || isLoadingRoles ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-500 hover:bg-green-600 transition-colors'}`}
                            >
                                {isSubmitting ? (
